...
export async function generateStaticParams() {
  const posts = getBlogPostsByMarket('uk')
...
export async function generateMetadata({ params }: BlogPostPageProps) {
  ...
  const post = getBlogPostBySlug(slug, 'uk')
  
  if (!post) {
    return {
      title: 'Blog Post Not Found | ARMH & Co UK',
    }
  }

  return {
    title: `${post.title} | ARMH & Co UK`,
            ...
            <div className="mt-16 pt-8 border-t border-charcoal-200">
              <div className="bg-gradient-to-br from-gold-faint to-white p-8 rounded-xl border border-gold-primary/20">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <Link href="/uk/contact" className="inline-block bg-gold-primary text-white px-8 py-3 rounded-lg font-semibold hover:bg-gold-secondary transition-colors tracking-wide font-sans text-center">
                    Book Free Consultation
                  </Link>
                  <Link href="/uk/resources" className="inline-block border border-gold-primary text-gold-primary px-8 py-3 rounded-lg font-semibold hover:bg-gold-faint transition-colors tracking-wide font-sans text-center">
                    View Free Resources
                  </Link>
                </div>
              </div>
